    ...
    protected function _getMethods(Behavior $instance, $class, $alias) {
        ...
        foreach ($finders as $finder => $methodName) {
            if (isset($this->_finderMap[$finder])) {
                ...
                $error = sprintf(
                        '%s contains duplicate finder "%s" which is already provided by "%s"', $class, $finder, $duplicate[0]
                );
        ...
        foreach ($methods as $method => $methodName) {
            if (isset($this->_methodMap[$method])) {
                ...
                $error = sprintf(
                        '%s contains duplicate method "%s" which is already provided by "%s"', $class, $method, $duplicate[0]
                );
                throw new Exception($error);
            }
