...
class Table implements RepositoryInterface, EventListener {
    ...
    public function findList(Query $query, array $options) {
        ...
        $options = $this->_setFieldMatchers(
                $options, ['idField', 'valueField', 'groupField']
        );

        return $query->formatResults(function($results) use ($options) {
                    return $results->combine(
                                    $options['idField'], $options['valueField'], $options['groupField']
                    );
                });
    }
    ...
    public function get($primaryKey, $options = []) {
        ...
        if (!$entity) {
            throw new RecordNotFoundException(sprintf(
                    'Record "%s" not found in table "%s"', implode(',', (array) $primaryKey), $this->table()
            ));
        }
    ...
    protected function _processSave($entity, $options) {
        ...
        $saved = $this->_associations->saveParents(
                $this, $entity, $options['associated'], $options->getArrayCopy()
        );
        ...
        $makeConditions = function($fields, $args) {
            ...
            if (count($args) < count($fields)) {
                throw new \Cake\Error\Exception(sprintf(
                        'Not enough arguments to magic finder. Got %s required %s', count($args), count($fields)
                ));
            }
            foreach ($fields as $field) {
