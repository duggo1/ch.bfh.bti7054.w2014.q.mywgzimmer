    ...
    public static function setCost($cost) {
        if ($cost < 4 || $cost > 31) {
            throw new Exception(vsprintf(
                    'Invalid value, cost must be between %s and %s', array(4, 31)
            ));
        }
    ...
    protected static function _crypt($password, $salt = false) {
        ...
        if ($salt === true || strpos($salt, '$2y$') !== 0 || strlen($salt) < 29) {
            throw new Exception(sprintf(
                    'Invalid salt: %s for blowfish Please visit http://www.php.net/crypt and read the appropriate section for building blowfish salts.', $salt
            ));
        }
