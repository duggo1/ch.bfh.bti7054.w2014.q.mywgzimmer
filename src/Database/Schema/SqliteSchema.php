...
class SqliteSchema extends BaseSchema {
    ...
    public function describeTableSql($name, $config) {
        $sql = sprintf(
                'PRAGMA table_info(%s)', $this->_driver->quoteIdentifier($name)
        );
        return [$sql, []];
    }
    ...
    public function describeIndexSql($table, $config) {
        $sql = sprintf(
                'PRAGMA index_list(%s)', $this->_driver->quoteIdentifier($table)
        );
        return [$sql, []];
    }
    ...
    public function convertIndexDescription(Table $table, $row) {
        $sql = sprintf(
                'PRAGMA index_info(%s)', $this->_driver->quoteIdentifier($row['name'])
        );
    ...
    public function constraintSql(Table $table, $name) {
        ...
        if ($data['type'] === Table::CONSTRAINT_FOREIGN) {
            $type = 'FOREIGN KEY';
            $clause = sprintf(
                    ' REFERENCES %s (%s) ON UPDATE %s ON DELETE %s', $this->_driver->quoteIdentifier($data['references'][0]), $this->_driver->quoteIdentifier($data['references'][1]), $this->_foreignOnClause($data['update']), $this->_foreignOnClause($data['delete'])
            );
        }
        $columns = array_map(
