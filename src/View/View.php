                ...
                case static::TYPE_ELEMENT:
                    ...
                    if (!$parent) {
                        ...
                        throw new \LogicException(sprintf(
                                'You cannot extend an element which does not exist (%s).', $defaultPath . $name . $this->_ext
                        ));
                    }
                    break;
    ...
    protected function _render($viewFile, $data = array()) {
        ...
        if ($initialBlocks !== $remainingBlocks) {
            throw new Exception(sprintf(
                    'The "%s" block was left open. Blocks are not allowed to cross files.', $this->Blocks->active()
            ));
        }
        return $content;
    }
