...
class CacheHelper extends Helper {
    ...
    public function afterLayout($event, $layoutFile) {
        if ($this->_enabled()) {
            $this->_View->assign(
                    'content', $this->cache($layoutFile, $this->_View->fetch('content'))
            );
        }
        $this->_View->assign(
                'content', preg_replace('/<!--\/?nocache-->/', '', $this->_View->fetch('content'))
        );
    }
    ...
    public function cache($file, $out) {
        ...
        if ($cacheTime && $cacheTime > 0) {
            ...
            try {
                ...
                $message = sprintf(
                        'Unable to write view cache file: "%s" for "%s"', $e->getMessage(), $this->request->here
                );
    ...
    protected function _parseFile($file, $cache) {
        ...
        preg_match_all(
                '/(?<=<!--nocache-->)([\\s\\S]*?)(?=<!--\/nocache-->)/i', $file, $fileResult, PREG_PATTERN_ORDER
        );
