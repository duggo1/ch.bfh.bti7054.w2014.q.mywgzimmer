...
class PaginatorHelper extends Helper {
    ...
    public function beforeRender($event, $viewFile) {
        $this->config(
                'options.url', array_merge($this->request->params['pass'], $this->request->query)
        );
    }
    ...
    protected function _toggledLink($text, $enabled, $options, $templates) {
        ...
        $url = array_merge(
                $options['url'], ['page' => $paging['page'] + $options['step']]
        );
    ...
    public function sort($key, $title = null, array $options = []) {
        $options = array_merge(
                ['url' => array(), 'model' => null, 'escape' => true], $options
        );
        $url = $options['url'];
        unset($options['url']);
        ...
        $url = array_merge(
                ['sort' => $key, 'direction' => $dir], $url, ['order' => null]
        );
        $vars = [
    ...
    public function first($first = '<< first', array $options = []) {
        $options = array_merge(
                ['model' => $this->defaultModel(), 'escape' => true], $options
        );
