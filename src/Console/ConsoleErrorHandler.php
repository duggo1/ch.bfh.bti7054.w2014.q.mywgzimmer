...
class ConsoleErrorHandler extends BaseErrorHandler {
    ...
    protected function _displayException($exception) {
        ...
        $message = sprintf(
                "<error>%s</error> %s in [%s, line %s]", $errorName, $exception->getMessage(), $exception->getFile(), $exception->getLine()
        );
    ...
    protected function _displayError($error, $debug) {
        $message = __d('cake_console', '%s in [%s, line %s]', $error['description'], $error['file'], $error['line']
        );
