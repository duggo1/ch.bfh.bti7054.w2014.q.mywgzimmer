...
class ModelTask extends BakeTask {
    ...
    public function bake($name) {
        ...
        $data = compact(
                'associations', 'primaryKey', 'displayField', 'table', 'fields', 'validation', 'behaviors'
        );
    ...
    public function getAssociations(Table $table) {
        ...
        if (is_array($primary) && count($primary) > 1) {
            $this->err(__d(
                            'cake_console', '<warning>Bake cannot generate associations for composite primary keys at this time</warning>.'
            ));
            return $associations;
        }
    ...
    protected function _getAllTables() {
        ...
        if (!method_exists($db, 'schemaCollection')) {
            $this->err(__d(
                            'cake_console', 'Connections need to implement schemaCollection() to be used with bake.'
            ));
            return $this->_stop();
        }
