...
class TestFixture {
    ...
    public function __construct() {
        ...
        if (!empty($this->connection)) {
            ...
            if (strpos($connection, 'test') !== 0) {
                $message = sprintf(
                        'Invalid datasource name "%s" for "%s" fixture. Fixture datasource names must begin with "test".', $connection, $this->name
                );
    ...
    protected function _schemaFromImport() {
        ...
        $import = array_merge(
                array('connection' => 'default', 'table' => null), $this->import
        );
    ...
    public function create(Connection $db) {
        ...
        try {
            ...
            $msg = sprintf(
                    'Fixture creation for "%s" failed "%s"', $this->table, $e->getMessage()
            );
            Log::error($msg);
