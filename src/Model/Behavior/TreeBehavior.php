...
class TreeBehavior extends Behavior {
    ...
    protected function _setParent($entity, $parent) {
        ...
        if ($parentLeft > $left && $parentLeft < $right) {
            throw new \RuntimeException(sprintf(
                    'Cannot use node "%s" as parent for entity "%s"', $parent, $entity->get($this->_table->primaryKey())
            ));
        }
    ...
    protected function _removeFromTree($node) {
        ...
        $this->_table->updateAll(
                [$config['parent'] => $parent], [$config['parent'] => $node->get($primary)]
        );
    ...
    protected function _recoverTree($counter = 0, $parentId = null) {
        ...
        $this->_table->updateAll(
                [$left => $leftCounter, $right => $counter + 1], [$pk[0] => $parentId]
        );
