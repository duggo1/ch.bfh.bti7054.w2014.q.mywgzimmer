...
class RouteTest extends TestCase {
    ...
    public function testRouteParsingWithExtensions() {
        $route = new Route(
                '/:controller/:action/*', array(), array('_ext' => array('json', 'xml'))
        );
    ...
    public function testMatchWithHostKeys() {
        ...
        $result = $route->match(
                array('controller' => 'posts', 'action' => 'index', '_host' => 'example.com'), $context
        );
        $this->assertEquals('http://example.com/posts/index', $result);

        $result = $route->match(
                array('controller' => 'posts', 'action' => 'index', '_scheme' => 'webcal'), $context
        );
        $this->assertEquals('webcal://foo.com/posts/index', $result);

        $result = $route->match(
                array('controller' => 'posts', 'action' => 'index', '_port' => '8080'), $context
        );
        $this->assertEquals('http://foo.com:8080/posts/index', $result);

        $result = $route->match(
                array('controller' => 'posts', 'action' => 'index', '_base' => '/dir'), $context
        );
    ...
    public function testGetNamePlugins() {
        $route = new Route(
                '/a/:controller/:action', array('plugin' => 'asset')
        );
        $this->assertEquals('asset._controller:_action', $route->getName());

        $route = new Route(
                '/a/assets/:action', array('plugin' => 'asset', 'controller' => 'assets')
        );
        $this->assertEquals('asset.assets:_action', $route->getName());

        $route = new Route(
                '/assets/get', array('plugin' => 'asset', 'controller' => 'assets', 'action' => 'get')
        );
