...
class RequestHandlerComponentTest extends TestCase {
    ...
    public function testRespondAsWithAttachment() {
        $this->RequestHandler = $this->getMock(
                'Cake\Controller\Component\RequestHandlerComponent', array('_header'), array($this->Controller->components())
        );
    ...
    public function testPrefers() {
        $this->RequestHandler->request->env(
                'HTTP_ACCEPT', 'text/xml,application/xml,application/xhtml+xml,text/html,text/plain,image/png,*/*'
        );
    ...
    public function testAjaxRedirectAsRequestAction() {
        ...
        $this->Controller->RequestHandler->beforeRedirect(
                $event, array('controller' => 'request_handler_test', 'action' => 'destination'), $this->Controller->response
        );
        $result = ob_get_clean();
    ...
    public function testAjaxRedirectAsRequestActionStillRenderingLayout() {
        ...
        ob_start();
        $this->Controller->RequestHandler->beforeRedirect(
                $event, array('controller' => 'request_handler_test', 'action' => 'ajax2_layout'), $this->Controller->response
        );
        $result = ob_get_clean();
        $this->assertRegExp('/posts index/', $result, 'RequestAction redirect failed.');
    ...
    public function testBeforeRedirectCallbackWithArrayUrl() {
        ...
        $RequestHandler->beforeRedirect(
                $event, array('controller' => 'request_handler_test', 'action' => 'param_method', 'first', 'second'), $this->Controller->response
        );
        $result = ob_get_clean();
