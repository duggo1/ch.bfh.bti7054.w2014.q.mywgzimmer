...
class ErrorHandlerTest extends TestCase {
    ...
    public function testHandleErrorLoggingTrace() {
        ...
        $this->_logger->expects($this->once())
                ...
                ->with('notice', $this->logicalAnd(
                                $this->stringContains('Notice (8): Undefined variable: out in '), $this->stringContains('Trace:'), $this->stringContains(__NAMESPACE__ . '\ErrorHandlerTest::testHandleErrorLoggingTrace()')
        ));
    ...
    public function testHandleExceptionLog() {
        ...
        $this->_logger->expects($this->once())
                ...
                ->with('error', $this->logicalAnd(
                                $this->stringContains('[Cake\Error\NotFoundException] Kaboom!'), $this->stringContains('ErrorHandlerTest->testHandleExceptionLog')
        ));
    ...
    public function testHandleExceptionLogSkipping() {
        ...
        $this->_logger->expects($this->once())
                ->method('write')
                ->with(
                        'error', $this->stringContains('[Cake\Error\ForbiddenException] Fooled you!')
        );
    ...
    public function testHandleFatalErrorLog() {
        $this->_logger->expects($this->at(0))
                ->method('write')
                ->with('error', $this->logicalAnd(
                                $this->stringContains(__FILE__ . ', line ' . (__LINE__ + 10)), $this->stringContains('Fatal Error (1)'), $this->stringContains('Something wrong')
        ));
