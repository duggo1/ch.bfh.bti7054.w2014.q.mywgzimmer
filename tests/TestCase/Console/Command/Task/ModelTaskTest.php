...
class ModelTaskTest extends TestCase {
    ...
    protected function _useMockedOut() {
        ...
        $this->Task = $this->getMock('Cake\Console\Command\Task\ModelTask', array('in', 'out', 'err', 'hr', 'createFile', '_stop', '_checkUnitTest'), array($io)
        );
        $this->_setupOtherMocks();
    }
    ...
    public function testBakeTableWithPlugin() {
        ...
        $this->Task->expects($this->once())->method('createFile')
                ->with($path, $this->logicalAnd(
                                $this->stringContains('namespace ControllerTest\\Model\\Table;'), $this->stringContains('use Cake\\ORM\\Table;'), $this->stringContains('class BakeArticlesTable extends Table {')
        ));
    ...
    public function testBakeEntityWithPlugin() {
        ...
        $this->Task->expects($this->once())->method('createFile')
                ->with($path, $this->logicalAnd(
                                $this->stringContains('namespace ControllerTest\\Model\\Entity;'), $this->stringContains('use Cake\\ORM\\Entity;'), $this->stringContains('class BakeArticle extends Entity {')
        ));
