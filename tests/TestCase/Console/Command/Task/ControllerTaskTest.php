...
class ControllerTaskTest extends TestCase {
    ...
    public function setUp() {
        ...
        $this->Task = $this->getMock('Cake\Console\Command\Task\ControllerTask', array('in', 'out', 'err', 'hr', 'createFile', '_stop'), array($io)
        );
        ...
        $this->Task->Model = $this->getMock('Cake\Console\Command\Task\ModelTask', array('in', 'out', 'err', 'createFile', '_stop'), array($io)
        );
        $this->Task->Test = $this->getMock(
                'Cake\Console\Command\Task\TestTask', [], [$io]
        );
    ...
    public function testBakeActions() {
        ...
        $this->Task->expects($this->at(1))
                ->method('createFile')
                ->with(
                        $this->_normalizePath($filename), $this->stringContains('class BakeArticlesController')
        );
        $result = $this->Task->bake('BakeArticles');
    ...
    public function testBakeWithPlugin() {
        ...
        $this->Task->expects($this->at(1))
                ->method('createFile')
                ->with(
                        $this->_normalizePath($path), $this->stringContains('BakeArticlesController extends AppController')
                )->will($this->returnValue(true));
    ...
    public function testMainIntoAll() {
        ...
        $this->Task->expects($this->at(1))
                ...
                ->with($filename, $this->logicalAnd(
                                $this->stringContains('class BakeArticlesController'), $this->stringContains("\$helpers = ['Time', 'Text']")
                ))
                ->will($this->returnValue(true));
