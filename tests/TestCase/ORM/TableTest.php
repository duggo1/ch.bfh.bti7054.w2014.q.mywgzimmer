...
class TableTest extends \Cake\TestSuite\TestCase {
    ...
    public function testSchema() {
        ...
        $table->schema($schema);
        $this->assertEquals(
                new \Cake\Database\Schema\Table('another', $schema), $table->schema()
        );
    }
    ...
    public function testUpdateAllFailure() {
        $table = $this->getMock(
                'Cake\ORM\Table', ['query'], [['table' => 'users', 'connection' => $this->connection]]
        );
    ...
    public function testFindApplyOptions() {
        $table = $this->getMock(
                'Cake\ORM\Table', ['query', 'findAll'], [['table' => 'users', 'connection' => $this->connection]]
        );
    ...
    public function testTableClassInPlugin() {
        ...
        $table = new Table();
        $this->assertEquals(
                'MyPlugin\Model\Entity\SuperUser', $table->entityClass('MyPlugin.SuperUser')
        );
    }
    ...
    public function testReciprocalBelongsToMany() {
        ...
        $this->assertInstanceOf(
                'TestApp\Model\Entity\ArticlesTag', $result->tags[0]->_joinData
        );
    }
    ...
    public function testImplementedEvents() {
        $table = $this->getMock(
                'Cake\ORM\Table', ['beforeFind', 'beforeSave', 'afterSave', 'beforeDelete', 'afterDelete']
        );
    ...
    public function testAfterSaveNotCalled() {
        $table = $this->getMock(
                '\Cake\ORM\Table', ['query', 'exists'], [['table' => 'users', 'connection' => $this->connection]]
        );
        $query = $this->getMock(
                '\Cake\ORM\Query', ['execute', 'addDefaultTypes'], [null, $table]
        );
    ...
    public function testAtomicSave() {
        ...
        $connection = $this->getMock(
                '\Cake\Database\Connection', ['begin', 'commit'], [$config]
        );
    ...
    public function testAtomicSaveRollback() {
        $connection = $this->getMock(
                '\Cake\Database\Connection', ['begin', 'rollback'], [ConnectionManager::config('test')]
        );
        $connection->driver(ConnectionManager::get('test')->driver());
        $table = $this->getMock(
                '\Cake\ORM\Table', ['query', 'connection', 'exists'], [['table' => 'users']]
        );
        $query = $this->getMock(
                '\Cake\ORM\Query', ['execute', 'addDefaultTypes'], [null, $table]
        );
    ...
    public function testSaveUpdateWithHint() {
        $table = $this->getMock(
                '\Cake\ORM\Table', ['exists'], [['table' => 'users', 'connection' => ConnectionManager::get('test')]]
        );
        $entity = new \Cake\ORM\Entity([
            'id' => 2,
            'username' => 'baggins'
    ...
    public function testSaveUpdatePrimaryKeyNotModified() {
        ...
        $query = $this->getMock(
                '\Cake\ORM\Query', ['execute', 'addDefaultTypes', 'set'], [null, $table]
        );
    ...
    public function testDeleteCallbacks() {
        ...
        $mock->expects($this->at(1))
                ...
                ->with($this->logicalAnd(
                                $this->attributeEqualTo('_name', 'Model.beforeDelete'), $this->attributeEqualTo(
                                        'data', ['entity' => $entity, 'options' => $options]
                                )
        ));
        ...
        $mock->expects($this->at(2))
                ->method('dispatch')
                ->with($this->logicalAnd(
                                $this->attributeEqualTo('_name', 'Model.afterDelete'), $this->attributeEqualTo(
                                        'data', ['entity' => $entity, 'options' => $options]
                                )
        ));
    ...
    public function testDeleteIsNew() {
        ...
        $table = $this->getMock(
                'Cake\ORM\Table', ['query'], [['connection' => $this->connection]]
        );
    ...
    public function testMagicFindAllAnd() {
        ...
        $expected = new QueryExpression(
                ['author_id' => 1, 'published' => 'Y'], $this->usersTypeMap
        );
        $this->assertEquals($expected, $result->clause('where'));
    }
    ...
    public function testSaveCleanEntity() {
        ...
        $entity = new \Cake\ORM\Entity(
                ['id' => 'foo'], ['markNew' => false, 'markClean' => true]
        );
    ...
    public function testSaveDeepAssociationOptions() {
        $articles = $this->getMock(
                '\Cake\ORM\Table', ['_insert'], [['table' => 'articles', 'connection' => $this->connection]]
        );
        $authors = $this->getMock(
                '\Cake\ORM\Table', ['_insert', 'validate'], [['table' => 'authors', 'connection' => $this->connection]]
        );
        $supervisors = $this->getMock(
                '\Cake\ORM\Table', ['_insert', 'validate'], [[
        'table' => 'authors',
        'alias' => 'supervisors',
        'connection' => $this->connection
            ]]
        ...
        $tags = $this->getMock(
                '\Cake\ORM\Table', ['_insert'], [['table' => 'tags', 'connection' => $this->connection]]
        );
    ...
    public function testGet() {
        $table = $this->getMock(
                '\Cake\ORM\Table', ['callFinder', 'query'], [[
        'connection' => $this->connection,
        'schema' => [
            'id' => ['type' => 'integer'],
            'bar' => ['type' => 'integer'],
        ...
        $query = $this->getMock(
                '\Cake\ORM\Query', ['addDefaultTypes', 'first', 'where'], [$this->connection, $table]
        );
