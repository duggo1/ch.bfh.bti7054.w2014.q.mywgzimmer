...
class EagerLoaderTest extends TestCase {
    ...
    public function testNormalizedPath() {
        ...
        $query = $this->getMock(
                '\Cake\ORM\Query', ['join'], [$this->connection, $this->table]
        );
        ...
        $this->assertEquals(
                'clients.orders.stuff.stuffTypes', $assocs['stuffTypes']['aliasPath']
        );
        $this->assertEquals(
                'client.order.stuff.stuff_type', $assocs['stuffTypes']['propertyPath']
        );
    }
    ...
    protected function _quoteArray($elements) {
        if ($this->connection->driver()->autoQuoting()) {
            ...
            return array_combine(
                    array_map($quoter, array_keys($elements)), array_map($quoter, array_values($elements))
            );
        }
        return $elements;
    }
