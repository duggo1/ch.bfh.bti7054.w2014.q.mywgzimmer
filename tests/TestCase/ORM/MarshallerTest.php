...
class MarshallerTest extends TestCase {
    ...
    public function testOneBelongsToManyJoinData() {
        ...
        $this->assertInstanceOf(
                'Cake\ORM\Entity', $result->tags[0]->_joinData, '_joinData should be an entity.'
        );
        $this->assertEquals(
                $data['tags'][0]['_joinData']['active'], $result->tags[0]->_joinData->active, '_joinData should be an entity.'
        );
    }
    ...
    public function testOneBelongsToManyJoinDataAssociated() {
        ...
        $this->assertInstanceOf(
                'Cake\ORM\Entity', $result->tags[0]->_joinData->user, 'joinData should contain a user entity.'
        );
        $this->assertEquals('Bill', $result->tags[0]->_joinData->user->username);
        $this->assertInstanceOf(
                'Cake\ORM\Entity', $result->tags[1]->_joinData->user, 'joinData should contain a user entity.'
        );
    ...
    public function testOneDeepAssociations() {
        ...
        $this->assertEquals(
                $data['article']['title'], $result->article->title
        );
        $this->assertEquals(
                $data['article']['user']['username'], $result->article->user->username
        );
    }
    ...
    public function testManyAssociations() {
        ...
        $this->assertInstanceOf('Cake\ORM\Entity', $result[1]);
        $this->assertEquals(
                $data[0]['user']['username'], $result[0]->user->username
        );
        $this->assertEquals(
                $data[1]['user']['username'], $result[1]->user->username
        );
    }
    ...
    public function testMergeMultipleAssociations() {
        ...
        $this->assertEquals(
                ['comment' => 'Extra comment 3', 'id' => 3], $entity->comments[2]->toArray()
        );
        $this->assertEquals(
                ['comment' => 'Extra comment 1'], $entity->comments[3]->toArray()
        );
        $this->assertEquals(
                ['comment' => 'Extra comment 2'], $entity->comments[4]->toArray()
        );
    }
    ...
    public function testMergeBelongsToManyJoinData() {
        ...
        $this->assertSame(
                ['active' => 0, 'foo' => 'bar'], $entity->tags[0]->_joinData->toArray()
        );
        $this->assertSame(
                ['active' => 1, 'foo' => 'baz'], $entity->tags[1]->_joinData->toArray()
        );
    ...
    public function testMergeComplexType() {
        $entity = new Entity(
                ['comment' => 'My Comment text'], ['markNew' => false, 'markClean' => true]
        );
        $data = [
            'created' => [
                'year' => '2014',
