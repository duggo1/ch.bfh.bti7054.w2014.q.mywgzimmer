...
class AssociationsTest extends TestCase {
    ...
    public function testSaveParents() {
        ...
        $mockOne = $this->getMock(
                'Cake\ORM\Association\BelongsTo', ['save'], ['Parent', [
                'sourceTable' => $table,
        ]]);
        $mockTwo = $this->getMock(
                'Cake\ORM\Association\HasMany', ['save'], ['Child', [
                'sourceTable' => $table
        ]]);
        ...
        $result = $this->associations->saveParents(
                $table, $entity, ['Parent', 'Child'], $options
        );
    ...
    public function testSaveParentsFiltered() {
        $table = $this->getMock('Cake\ORM\Table', [], [[]]);
        $mockOne = $this->getMock(
                'Cake\ORM\Association\BelongsTo', ['save'], ['Parents', [
                'sourceTable' => $table,
        ]]);
        $mockTwo = $this->getMock(
                'Cake\ORM\Association\BelongsTo', ['save'], ['Categories', [
                'sourceTable' => $table
        ]]);
        ...
        $result = $this->associations->saveParents(
                $table, $entity, ['Parents' => ['associated' => ['Others']]], $options
        );
        $this->assertTrue($result, 'Save should work.');
    }
    ...
    public function testSaveChildrenFiltered() {
        $table = $this->getMock('Cake\ORM\Table', [], [[]]);
        $mockOne = $this->getMock(
                'Cake\ORM\Association\HasMany', ['save'], ['Comments', [
                'sourceTable' => $table,
        ]]);
        $mockTwo = $this->getMock(
                'Cake\ORM\Association\HasOne', ['save'], ['Profiles', [
                'sourceTable' => $table
        ]]);
        ...
        $result = $this->associations->saveChildren(
                $table, $entity, ['Comments' => ['associated' => ['Other']]], $options
        );
    ...
    public function testErrorOnUnknownAlias() {
        $table = $this->getMock(
                'Cake\ORM\Table', ['save'], [['alias' => 'Users']]);

        $entity = new Entity();
        $entity->set('profile', ['key' => 'value']);

        $this->associations->saveChildren(
                $table, $entity, ['Profiles'], ['atomic' => true]
        );
    }
