...
class BelongsToManyTest extends TestCase {
    ...
    public function testEagerLoaderSubquery() {
        ...
        $query = $this->getMock(
                'Cake\ORM\Query', ['all', 'where', 'andWhere', 'order', 'select', 'matching'], [null, $this->article]
        );
    ...
    public function testEagerLoaderWithQueryBuilder() {
        ...
        $keys = [1, 2, 3, 4];
        $query = $this->getMock(
                'Cake\ORM\Query', ['all', 'matching', 'andWhere', 'limit'], [null, null]
        );
    ...
    public function testLinkSuccess() {
        ...
        $joint = $this->getMock(
                '\Cake\ORM\Table', ['save'], [['alias' => 'ArticlesTags', 'connection' => $connection]]
        );
        $config = [
            'sourceTable' => $this->article,
            'targetTable' => $this->tag,
    ...
    public function testUnlinkSuccess() {
        $connection = ConnectionManager::get('test');
        $joint = $this->getMock(
                '\Cake\ORM\Table', ['delete', 'find'], [['alias' => 'ArticlesTags', 'connection' => $connection]]
        );
        $config = [
            'sourceTable' => $this->article,
            'targetTable' => $this->tag,
    ...
    public function testReplaceLinkSuccess() {
        ...
        $assoc = $this->getMock(
                '\Cake\ORM\Association\BelongsToMany', ['_collectJointEntities', '_saveTarget'], ['tags', $config]
        );
        $assoc->junction();
        ...
        $query1 = $this->getMock(
                '\Cake\ORM\Query', ['where', 'andWhere', 'addDefaultTypes'], [$connection, $joint]
        );
    ...
    public function testSaveWithReplace() {
        $assoc = $this->getMock(
                '\Cake\ORM\Association\BelongsToMany', ['replaceLinks'], ['tags']
        );
        $entity = new Entity([
            'id' => 1,
            'tags' => [
    ...
    public function testSaveOnlyEntities() {
        $connection = ConnectionManager::get('test');
        $mock = $this->getMock(
                'Cake\ORM\Table', ['save', 'schema'], [['table' => 'tags', 'connection' => $connection]]
        );
        $mock->primaryKey('id');
    ...
    public function testAttachToBeforeFind() {
        ...
        $listener->expects($this->once())->method('__invoke')
                ->with(
                        $this->isInstanceOf('\Cake\Event\Event'), $this->isInstanceOf('\Cake\ORM\Query'), [], false
        );
    ...
    public function testAttachToBeforeFindExtraOptions() {
        ...
        $listener->expects($this->once())->method('__invoke')
                ->with(
                        $this->isInstanceOf('\Cake\Event\Event'), $this->isInstanceOf('\Cake\ORM\Query'), $opts, false
        );
