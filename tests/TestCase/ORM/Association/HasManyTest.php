...
class HasManyTest extends \Cake\TestSuite\TestCase {
    ...
    public function setUp() {
        ...
        $this->article = $this->getMock(
                'Cake\ORM\Table', ['find', 'deleteAll', 'delete'], [['alias' => 'Articles', 'table' => 'articles']]
        );
        $this->article->schema([
            'id' => ['type' => 'integer'],
            'title' => ['type' => 'string'],
    ...
    public function testEagerLoaderWithDefaults() {
        ...
        $keys = [1, 2, 3, 4];
        $query = $this->getMock(
                'Cake\ORM\Query', ['all', 'where', 'andWhere', 'order'], [null, null]
        );
        $this->article->expects($this->once())->method('find')->with('all')
                ->will($this->returnValue($query));
        $results = [
    ...
    public function testEagerLoaderWithOverrides() {
        ...
        $keys = [1, 2, 3, 4];
        $query = $this->getMock(
                'Cake\ORM\Query', ['all', 'where', 'andWhere', 'order', 'select', 'contain'], [null, null]
        );
        $this->article->expects($this->once())->method('find')->with('all')
                ->will($this->returnValue($query));
        $results = [
    ...
    public function testEagerLoaderWithQueryBuilder() {
        ...
        $keys = [1, 2, 3, 4];
        $query = $this->getMock(
                'Cake\ORM\Query', ['all', 'select', 'join', 'where'], [null, null]
        );
        $this->article->expects($this->once())->method('find')->with('all')
                ->will($this->returnValue($query));
        $results = [
