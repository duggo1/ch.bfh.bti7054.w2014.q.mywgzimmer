...
class QueryTest extends TestCase {
    ...
    public function testMapReduceOnlyMapper() {
        ...
        $this->assertEquals(
                [['mapper' => $mapper1, 'reducer' => null]], $query->mapReduce()
        );
    ...
    public function testMapReduceBothMethods() {
        ...
        $this->assertEquals(
                [['mapper' => $mapper1, 'reducer' => $reducer1]], $query->mapReduce()
        );
    ...
    public function testOverwriteMapReduce() {
        ...
        $this->assertEquals(
                [['mapper' => $mapper2, 'reducer' => $reducer2]], $query->mapReduce()
        );
    }
    ...
    public function testResultsAreWrappedInMapReduce() {
        ...
        $statement = $this->getMock(
                '\Database\StatementInterface', ['fetch', 'closeCursor', 'rowCount']
        );
    ...
    public function testCollectionProxy($method, $arg) {
        $query = $this->getMock(
                '\Cake\ORM\Query', ['all'], [$this->connection, $this->table]
        );
        $query->select();
        ...
        $this->assertInstanceOf(
                '\Cake\Collection\Collection', $query->{$method}($arg, 'extra')
        );
    }
    ...
    public function testCacheReadIntegration() {
        $query = $this->getMock(
                '\Cake\ORM\Query', ['execute'], [$this->connection, $this->table]
        );
        $resultSet = $this->getMock('\Cake\ORM\ResultSet', [], [$query, null]);
    ...
    public function testCacheWriteIntegration() {
        ...
        $cacher->expects($this->once())
                ->method('write')
                ->with(
                        'my_key', $this->isInstanceOf('Cake\ORM\ResultSet')
        );
            ...
            public function testFormatDeepAssocationRecords() {
                ...
                $query->formatResults(function($results) {
                    return $results->map(function($row) {
                                return sprintf(
                                        '%s - %s - %s', $row->tag_id, $row->article->idCopy, $row->article->author->idCopy
                                );
                            });
                });
