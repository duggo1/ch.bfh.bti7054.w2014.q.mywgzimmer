...
TEXT;
    ...
    public function testLog() {
        ...
        $mock->expects($this->at(0))
                ...
                ->with('debug', $this->logicalAnd(
                                $this->stringContains('DebuggerTest::testLog'), $this->stringContains('cool')
        ));

        $mock->expects($this->at(1))
                ->method('write')
                ->with('debug', $this->logicalAnd(
                                $this->stringContains('DebuggerTest::testLog'), $this->stringContains('[main]'), $this->stringContains("'whatever',"), $this->stringContains("'here'")
        ));
    ...
    public function testLogDepth() {
        ...
        $mock->expects($this->at(0))
                ->method('write')
                ->with('debug', $this->logicalAnd(
                                $this->stringContains('DebuggerTest::testLog'), $this->stringContains('test'), $this->logicalNot($this->stringContains('val'))
        ));
