...
class SqlserverTest extends \Cake\TestSuite\TestCase {
    ...
    public function testConnectionConfigCustom() {
        ...
        $driver = $this->getMock(
                'Cake\Database\Driver\Sqlserver', ['_connect', 'connection'], [$config]
        );
    ...
    public function testSelectLimitVersion12() {
        $driver = $this->getMock(
                'Cake\Database\Driver\Sqlserver', ['_connect', 'connection', '_version'], [['dsn' => 'foo']]
        );
        $driver
                ->expects($this->any())
                ->method('_version')
        ...
        $connection = $this->getMock(
                '\Cake\Database\Connection', ['connect', 'driver'], [['log' => false]]
        );
        $connection
                ->expects($this->any())
                ->method('driver')
