...
class ConnectionTest extends TestCase {
    ...
    public function testInsertWithMatchingTypes() {
        ...
        $result = $this->connection->insert(
                'things', $data, ['id' => 'integer', 'title' => 'string', 'body' => 'string']
        );
        $this->assertInstanceOf('Cake\Database\StatementInterface', $result);
        $result->closeCursor();
    ...
    public function testInsertWithPositionalTypes() {
        $data = ['id' => '3', 'title' => 'a title', 'body' => 'a body'];
        $result = $this->connection->insert(
                'things', $data, ['integer', 'string', 'string']
        );
        $result->closeCursor();
        $this->assertInstanceOf('Cake\Database\StatementInterface', $result);
        $result = $this->connection->execute('SELECT * from things where id  = 3');
    ...
    public function testLogFunction() {
        ...
        $logger->expects($this->once())->method('log')
                ->with($this->logicalAnd(
                                $this->isInstanceOf('\Cake\Database\Log\LoggedQuery'), $this->attributeEqualTo('query', 'SELECT 1')
        ));
    ...
    public function testLogBeginRollbackTransaction() {
        ...
        $logger->expects($this->at(0))->method('log')
                ->with($this->logicalAnd(
                                $this->isInstanceOf('\Cake\Database\Log\LoggedQuery'), $this->attributeEqualTo('query', 'BEGIN')
        ));
        $logger->expects($this->at(1))->method('log')
                ->with($this->logicalAnd(
                                $this->isInstanceOf('\Cake\Database\Log\LoggedQuery'), $this->attributeEqualTo('query', 'ROLLBACK')
        ));
    ...
    public function testLogCommitTransaction() {
        ...
        $connection = $this->getMock(
                '\Cake\Database\Connection', ['connect'], [['driver' => $driver]]
        );
        ...
        $logger->expects($this->at(1))->method('log')
                ->with($this->logicalAnd(
                                $this->isInstanceOf('\Cake\Database\Log\LoggedQuery'), $this->attributeEqualTo('query', 'COMMIT')
        ));
        $connection->logQueries(true);
        $connection->begin();
        $connection->commit();
    ...
    public function testTransactionalSuccess() {
        $driver = $this->getMockFormDriver();
        $connection = $this->getMock(
                '\Cake\Database\Connection', ['connect', 'commit', 'begin'], [['driver' => $driver]]
        );
    ...
    public function testTransactionalFail() {
        $driver = $this->getMockFormDriver();
        $connection = $this->getMock(
                '\Cake\Database\Connection', ['connect', 'commit', 'begin', 'rollback'], [['driver' => $driver]]
        );
        $connection->expects($this->at(0))->method('begin');
