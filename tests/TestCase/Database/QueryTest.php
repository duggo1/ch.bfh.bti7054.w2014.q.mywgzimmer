...
class QueryTest extends TestCase {
    ...
    public function testSelectAliasedTables() {
        ...
        $result = $query->select(['name' => 'b.name'])->from(['b' => 'authors'])
                ...
                ->execute();
        $this->assertEquals(
                ['text' => 'Third Article Body', 'author_id' => 1, 'name' => 'nate'], $result->fetch('assoc')
        );
        $this->assertEquals(
                ['text' => 'Third Article Body', 'author_id' => 1, 'name' => 'mariano'], $result->fetch('assoc')
        );
    }
    ...
    public function testSelectWhereOperatorMethods() {
        ...
        $result = $query
                ...
                ->where(function($exp) {
                    return $exp->in(
                                    'created', [new \DateTime('2007-03-18 10:45:23'), new \DateTime('2007-03-18 10:47:23')], 'datetime'
                    );
                })
                ->execute();
        $this->assertCount(2, $result);
    ...
    public function testInValueCast() {
        ...
        $result = $query
                ...
                ->where(function($exp, $q) {
                    return $exp->in(
                                    'created', $q->newExpr()->add("'2007-03-18 10:45:23'"), 'datetime'
                    );
                })
                ->execute();
        $this->assertCount(1, $result);
    ...
    public function testSelectDistinctON() {
        $this->skipIf(
                $this->connection->driver() instanceof \Cake\Database\Driver\Sqlserver, 'Not implemented yet in SqlServer'
        );
        $query = new Query($this->connection);
        $result = $query
                ->select(['id', 'author_id'])
    ...
    public function testSelectModifiers() {
        ...
        $this->assertQuotedQuery(
                'SELECT DISTINCTROW <city>, <state>, <country> FROM <addresses>', $result->sql(), true
        );
        ...
        $this->assertQuotedQuery(
                'SELECT DISTINCTROW SQL_NO_CACHE <city>, <state>, <country> FROM <addresses>', $result->sql(), true
        );
        ...
        $result = $query
                ...
                ->modifier(['TOP 10']);
        $this->assertQuotedQuery(
                'SELECT TOP 10 <city>, <state>, <country> FROM <addresses>', $result->sql(), true
        );
    }
    ...
    public function testUpdateMultipleFields() {
        ...
        $this->assertQuotedQuery(
                'UPDATE <articles> SET <title> = :c0 , <body> = :c1', $result, true
        );
    ...
    public function testUpdateWithExpression() {
        ...
        $this->assertQuotedQuery(
                'UPDATE <articles> SET title = author_id WHERE <id> = :', $result, true
        );
    ...
    public function testUpdateArrayFields() {
        ...
        $this->assertQuotedQuery(
                'UPDATE <comments> SET <comment> = :c0 , <created> = :c1', $result, true
        );
    ...
    public function testInsertFromSelect() {
        ...
        $query = new Query($this->connection);
        $query->insert(
                        ['title', 'body', 'author_id'], ['title' => 'string', 'body' => 'string', 'author_id' => 'integer']
                )
                ->into('articles')
                ->values($select);
        ...
        $result = $query->sql();
        $this->assertQuotedQuery(
                'INSERT INTO <articles> \(<title>, <body>, <author_id>\) SELECT', $result, true
        );
        $this->assertQuotedQuery(
    ...
    public function testSQLFunctions() {
        ...
        $this->assertWithinMargin(
                date('U'), (new \DateTime($result->fetchAll('assoc')[0]['d']))->format('U'), 1
        );
