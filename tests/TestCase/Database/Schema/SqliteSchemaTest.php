...
SQL;
    ...
    public function testDescribeTableIndexes() {
        ...
        $this->assertEquals(
                $expected['sqlite_autoindex_schema_articles_1'], $result->constraint('sqlite_autoindex_schema_articles_1')
        );
        $this->assertEquals(
                $expected['author_id_fk'], $result->constraint('author_id_fk')
        );
...
SQL;
        ...
        $this->assertEquals(
                'CREATE INDEX "title_idx" ON "articles" ("title")', $result[1]
        );
    }
...
SQL;
    ...
    public function testTruncateSql() {
        ...
        $statement = $this->getMock(
                '\PDOStatement', ['execute', 'rowCount', 'closeCursor', 'fetch']
        );
