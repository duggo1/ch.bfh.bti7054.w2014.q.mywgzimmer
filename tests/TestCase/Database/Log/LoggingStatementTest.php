...
class LoggingStatementTest extends \Cake\TestSuite\TestCase {
    ...
    public function testExecuteNoParams() {
        ...
        $logger->expects($this->once())
                ...
                ->with($this->logicalAnd(
                                $this->isInstanceOf('\Cake\Database\Log\LoggedQuery'), $this->attributeEqualTo('query', 'SELECT bar FROM foo'), $this->attributeEqualTo('took', 5, 200), $this->attributeEqualTo('numRows', 3), $this->attributeEqualTo('params', [])
        ));
    ...
    public function testExecuteWithParams() {
        ...
        $logger->expects($this->once())
                ->method('log')
                ->with($this->logicalAnd(
                                $this->isInstanceOf('\Cake\Database\Log\LoggedQuery'), $this->attributeEqualTo('query', 'SELECT bar FROM foo'), $this->attributeEqualTo('took', 5, 5), $this->attributeEqualTo('numRows', 4), $this->attributeEqualTo('params', ['a' => 1, 'b' => 2])
        ));
        $st = new LoggingStatement($inner);
        $st->queryString = 'SELECT bar FROM foo';
        $st->logger($logger);
    ...
    public function testExecuteWithBinding() {
        ...
        $logger->expects($this->at(0))
                ->method('log')
                ->with($this->logicalAnd(
                                $this->isInstanceOf('\Cake\Database\Log\LoggedQuery'), $this->attributeEqualTo('query', 'SELECT bar FROM foo'), $this->attributeEqualTo('took', 5, 5), $this->attributeEqualTo('numRows', 4), $this->attributeEqualTo('params', ['a' => 1, 'b' => '2013-01-01'])
        ));
        $logger->expects($this->at(1))
                ->method('log')
                ->with($this->logicalAnd(
                                $this->isInstanceOf('\Cake\Database\Log\LoggedQuery'), $this->attributeEqualTo('query', 'SELECT bar FROM foo'), $this->attributeEqualTo('took', 5, 5), $this->attributeEqualTo('numRows', 4), $this->attributeEqualTo('params', ['a' => 1, 'b' => '2014-01-01'])
        ));
